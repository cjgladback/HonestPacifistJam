...
func _physics_process(delta: float) -> void:
	...
	move_direction.x = Input.get_action_strength("birdLleft") - Input.get_action_strength("birdLright") #reversed directions
...
#	_velocity.z = move_direction.z * speed
	...
	if _velocity.length() > 0.2:
		...
		_model.rotation.y = look_direction.angle() - deg2rad(180)
